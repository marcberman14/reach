...
if (isset($_POST['test_name'],$_POST['test_description'],$_POST['test_marks'])) {
    ...
    $array = array("testname"=>$test_name, "testdesc"=> $test_description, "testmarks"=> $test_marks, "testid"=> $testid);

    $test = new TestDao();
	
	$subjectid = $test->getTest(array("testid"=>$testid));
	
	$subjectid = $subjectid[0]['subject_id'];
    
    $test->updateTest($array);
